...
if (isset($_POST['submitform'])){
	...
	if (!empty($_POST['nama_lengkap']) && !empty($_POST['email']) && !empty($_POST['nomor_whatsapp']) && !empty($_POST['detail_masalah'])){
		...
		$queryform = "INSERT INTO hrform (nama_lengkap, email, nomor_whatsapp, detail_masalah) VALUES ('$nameform', '$emailform', '$nowhatsapp', '$masalah')" ;

    $run = mysqli_query($conn, $queryform) or die (mysqli_error());

    if ($run) {
      echo "<script>
			alert('Terimakasih telah mengisi form ini, tunggu pihak kami menghubungi anda');
			document.location.href='form-human-resource.php';
			</script>";
    } else {
      echo "<script>
    ...
    <div class="container-contact100">
      <div class="wrap-contact100">
        <form action="" method="POST" class="contact100-form validate-form" id="whatsapp">
          <span class="contact100-form-title">HUMAN RESOURCE FORM</span>
